...
class Sequential:
    ...
    def fit(self, X, y, epochs=10, batch_size=None):
        ...
        if batch_size is None:
            batch_size = num_samples  # Use the entire dataset as a batch
        ...
        for epoch in range(epochs):
            ...
            for batch in range(num_batches):
                ...
                if len(y_batch_labels.shape) == 2:
                    y_batch_labels = np.argmax(y_batch_labels, axis=1)
                epoch_accuracy += np.sum(predictions == y_batch_labels)
                ...
                progress_bar.set_postfix(loss=f"{np.mean(loss_batch):.4f}",
                                         acc=f"{np.mean(predictions == y_batch_labels):.4f}")
    ...
    def load_model(cls, filepath):
        ...
        # Create a new instance of Sequential
        model = Sequential()
        ...
        for layer in loaded_model.layers:
            ...
            elif class_name == 'Activation_ReLU' or class_name == 'ReLU' or isinstance(layer, Activation_ReLU):
                new_layer = Activation_ReLU()
            elif class_name == 'Activation_Softmax' or class_name == 'Softmax' or isinstance(layer, Activation_Softmax):
                new_layer = Activation_Softmax()
            elif class_name == 'Activation_Sigmoid' or class_name == 'Sigmoid' or isinstance(layer, Activation_Sigmoid):
                new_layer = Activation_Sigmoid()
            elif class_name == 'Activation_Linear' or class_name == 'Linear' or isinstance(layer, Activation_Linear):
                new_layer = Activation_Linear()
            elif class_name == 'Activation_Tanh' or class_name == 'Tanh' or isinstance(layer, Activation_Tanh):
